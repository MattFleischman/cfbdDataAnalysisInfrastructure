...
def post_prediction_adjustments(event, context):
    s3_bucket = os.environ['s3_source_bucket']
    output_prefix = event.get('output_file', post_forecast_adjustments_path)
    year_input = event.get('year', None)
    year = year_input if year_input else datetime.datetime.now().strftime('%Y')
    season_type = event.get('season_type', None)
    week = event.get('week', None)
    ...
    #pull forecast dependencies
    raw_forecast_df = utilities.dataframe_from_s3(
        f"{cfbd_prefix}{forecast_output_path}{ingest_prefix}", s3_bucket)
    ...
    game_line_output_df = post_pred_transformations.modify_game_line_output(raw_forecast_df, betting_lines_df)
